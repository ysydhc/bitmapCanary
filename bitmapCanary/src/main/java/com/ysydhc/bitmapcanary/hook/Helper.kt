package com.ysydhc.bitmapcanary.hook
...
internal object Helper {
    ...
    fun getFragmentsInActivity(activity: Activity?): List<Fragment> {
        ...
        if (activity.javaClass.name.equals("androidx.fragment.app.FragmentActivity")) {
    ...
    fun getParentFragments(f: Fragment): List<Fragment> {
        ...
        while (curFragment.parentFragment != null) {
            val pf = curFragment.requireParentFragment()
            pfs.add(pf)
            curFragment = pf
        }
        return pfs
    ...
    fun canIgnore(clazzName: String?, viewName: String?): Boolean {
        ...
        val viewNames = ignoreClassMap[clazzName]
